    ...
    def plot_points(self,im,points,colors = None,sizes = None):
        ...
        for pidx,point in enumerate(im_points):
            if colors is not None:
                ...
                color_int = colors[pidx]
                color = (int(color_int[0]),int(color_int[1]),int(color_int[2]))
                # print(type(color))
...
if __name__ == "__main__":
    ...
    # p = pd.read_csv('/Volumes/SSD_MNICE/video_radar_data.csv')
    p = pd.read_csv('./labeled_homograph_radar.csv')
    ...
    path = "./test_homography.MP4"
    t0 = 1628251882 #corresponds to 13:11:21 in video (1 sec off t_csv?)
    # t0_short = 1628251880+120+1 #corresponds to 13:13:20 in video
    t = t0
    cap = cv2.VideoCapture(path)
    ret,frame = cap.read()

    cmap = pt.get_cmap('hsv')
    number = np.max(p.ID)+1
    ...
    last_color = 0
    spaced_colors = []
    for i in p.ID.unique():
        spaced_colors.append(all_colors[np.mod(last_color,number)])
        last_color += 15

    # colors = np.zeros((len(ids),3))
    # for i in range(0,len(colors)):
    #     colors[i] = spaced_colors[ids[i]-1][:3]


    while ret:
        # 30 FPS Video
        # hg.plot_axes(frame)
        # plot points from the next 1 second?
        one_second = p.loc[
            (p.Time>t)&
            (p.Time<t+(3/30))
        ].reset_index(drop=True)
        points = np.zeros((len(one_second.SpaceGap),3))
        points[:,0] = one_second.SpaceGap.values*3.28 #convert to feet
        points[:,1] = one_second.Latitude.values*3.28 #convert to feet
        points = torch.tensor(points)
        points = points.unsqueeze(0)                           # points now has dimension `[1,100,3]`
        # print(points)
        size = points[0][:,2]+4

        #need to make an n x 3 array which has the colors for each index
        ids = p.ID.reset_index(drop=True)
        number = np.max(p.ID)+1
        cmap = pt.get_cmap('hsv')
        # all_colors = [cmap(i) for i in np.linspace(0, 1, number)]
        ...
        # print(one_second)
        for i in range(0,len(one_second)):
            # color[i] = all_colors[one_second.ID[i]][:3]
            color[i] = spaced_colors[one_second.ID[i]-1][:3]
            # print(color[i])
        color = (color*255).astype(int) #n x 3 array of colors for each data point
        ...
        hg.plot_points(frame,points.squeeze(0), colors = color, sizes = size)
